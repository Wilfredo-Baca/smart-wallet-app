  ...
  return (
    <div className="card">
      <div className="card-header">
        <section className="icon-title-card">
          ...
          <h3 className="text-xl">{meta.NombreMeta}</h3>
        </section>
        <hr className="hr-line hr-3" />
      </div>
      <div className="card-body">
        <p>
          <span className="point">&bull;</span>Dinero Objetivo:{" "}
          {formatCurrency(meta.MontoObjetivo)}
        </p>
        <p>
          <span className="point">&bull;</span>Dinero Ahorrado:{" "}
          {formatCurrency(meta.MontoAhorrado)}
        </p>
        <p>
          <span className="point">&bull;</span>Dinero Faltante:{" "}
          {formatCurrency(meta.MontoObjetivo - meta.MontoAhorrado)}
        </p>
        <p>
          <span className="point">&bull;</span>Tiempo Faltante: {howLong(meta.FechaMeta)}
        </p>
        <p>
          <span className="point">&bull;</span>Nivel de Prioridad:{" "}
          {meta.Prioridad}
        </p>
        <p>
          <span className="point">&bull;</span>Fecha Inicio: {getTime(meta.FechaInicio)}
        </p>
        <p>
          <span className="point">&bull;</span>Fecha Límite: {getTime(meta.FechaMeta)}
        </p>
        {meta.FechaProximaTransferencia && (
          <p>
            <span className="point">&bull;</span>Proxima Transferencia:{" "}
            {getTime(meta.FechaProximaTransferencia)}
          </p>
        )}
        {meta.MontoTransferencia && (
          <p>
            <span className="point">&bull;</span>Monto para Proxima Transferencia:{" "}
            {meta.MontoTransferencia}
          </p>
        )}
        <p>
          <span className="point">&bull;</span>Descripción:{" "}
