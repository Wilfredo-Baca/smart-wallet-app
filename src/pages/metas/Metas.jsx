...
import CardMetas from "../../components/card/CardMetas";
import Modal from "../../components/modal/Modal.jsx";
import FormMeta from "./FormMeta.jsx";
import getMetas from "../../services/getMetas.js";
import { getTime } from "../../utils/dateUtils.js";
import { useLocation } from "react-router-dom";
import { formatCurrency } from "../../utils/formatNum.js";
...
const misMetas = () => {
  ...
  useEffect(() => {
    ...
    getMetas().then((data) => setMetas(data));
  ...
  return (
    <>
      <section className="page-metas">
        ...
        <button className={metas[0] === "no" ? "btn-crear-meta btn-disabled" : "btn-crear-meta"} onClick={openModal}>
            Crear Meta
          </button>
        {metas?.length === 0 ? (
          <h2 className="no-metas">Obteniendo metas...</h2>) 
          : metas[0] === "No hay metas registradas" || metas[0] === "no" ? (
          <h2 className="no-metas">No hay metas registradas.</h2>
        ) : (
          metas?.map(
            ({
              ID_Meta,
              ...
              <CardMetas
                ...
                id={ID_Meta}
                title={NombreMeta}
                amount={formatCurrency(MontoObjetivo)}
                priority={Prioridad}
                startDate={getTime(FechaInicio)}
                endDate={getTime(FechaMeta)}
